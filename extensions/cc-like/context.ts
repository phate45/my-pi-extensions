 ...
 * - skills
 * - effective project context files
...
class ContextView implements Component {
	...
	private rebuild(width: number): void {
		...
		if (this.data.usage) {
			...
			lines.push(
				muted("System: ") +
					...
					muted(` (Context ~${u.agentTokens.toLocaleString()})`),
			);
			lines.push(
				muted("Tools: ") +
		...
		lines.push(muted(`Context (${this.data.agentFiles.length}): `) + text(this.data.agentFiles.length ? joinComma(this.data.agentFiles) : "(none)"));
		lines.push("");
...
export default function contextExtension(pi: ExtensionAPI) {
	if (!isManagedExtensionEnabled("context", "ccLike")) return;
	...
	pi.registerCommand("context", {
		...
		handler: async (_args, ctx: ExtensionCommandContext) => {
			...
			const skills = getSkillNames(pi);

			const agentFiles = loadEffectiveContextFiles(ctx.cwd);
			const agentFilePaths = agentFiles.map((f) => shortenPath(f.path, ctx.cwd));
			const agentTokens = agentFiles.reduce((a, f) => a + estimateTokens(f.content), 0);
			...
			const makePlainText = () => {
				...
				lines.push(`System: ~${systemPromptTokens.toLocaleString()} tok (Context ~${agentTokens.toLocaleString()})`);
				lines.push(`Tools: ~${toolsTokens.toLocaleString()} tok (${activeToolNames.length} active)`);
				lines.push(`Context: ${agentFilePaths.length ? joinComma(agentFilePaths) : "(none)"}`);
